...
Lifecycle: raw ingestion creates `current` in this compact implementation; clients that require review can use `set_lifecycle` for `observed`, `validated`, promotion to `current`, or `archived`. Replacements atomically mark the old current fact `superseded`, set `valid_to`, and insert a new `current` fact. `archived` is retained but excluded from normal lookup. No information is deleted by an update.
...
`tests/test_engine.py` covers alias resolution, ambiguous names, duplicate assertion observations, supersession, historical and point-in-time lookup, relationship traversal, temporal changes, persistence/index rebuild, and concurrent writes. Run:
