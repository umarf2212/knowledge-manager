---
name: mms-recall
description: Retrieve an exact current or historical personal fact from the local MMS Memory Engine. Use when the user writes /mms-recall followed by a question about remembered personal information, preferences, relationships, possessions, dates, or changes over time.
---

# MMS Recall

Interpret the text after `/mms-recall` as a deterministic memory query. Resolve the named entity and use the narrowest matching command. Return only relevant structured facts, phrased naturally; include source and dates when requested. Never scan or inject the whole database.
