...
Use `lookup` for the current state, `history` for all states, `at` for a historical point, and `changes` for facts observed in an interval. All command results include enough provenance to explain the match.
